...
for (i in length(years):2) {
  for (j in (i-1):1) {
    ...
    print(diff_name)
    BCPopulation[paste0("change_", diff_name)] = 
      BCPopulation[years[i]] - BCPopulation[years[j]]
    pct = ((1 / BCPopulation[years[j]]) * BCPopulation[years[i]]) - 1
    BCPopulation[paste0("pct_", diff_name)] = pct

    BCPopulation[paste0("colour_", diff_name)] = 
...
for (i in length(years):2) {
  for (j in (i-1):1) {
    ...
    print(pct_column)
    gradiant_out = paste0("output/", pct_column)
    ...
    colour_map <- ggplot() + geom_sf(data = pop_polygons, color = 'black', aes(fill = get(colour_column))) +
