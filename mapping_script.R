...
for (i in length(years):2){
  for (j in (i-1):1){
    ...
    gradiant_out = paste0("output/gradient/", pct_column, ".png")
    
    pct_map <- ggplot() + geom_sf(data = pop_polygons, aes(fill = get(pct_column))) +
    ...
    pct_map
    ggsave(plot = pct_map, width = 7, height = 6, dpi = 1200, filename = gradiant_out)
    
    colour_column = paste0("colour_", years[j], "_", years[i])
    classified_out = paste0("output/classified/", colour_column, ".png")
    
    colour_map <- ggplot() + geom_sf(data = pop_polygons, aes(fill = factor(get(colour_column)))) +
      scale_fill_manual(na.value = "grey50", values = c(low = low_colour, mid = mid_colour, high = high_colour),
                        labels = c(low_label, "", high_label)) +
      guides(fill=guide_legend(title=paste0("British Columbia\n%pop. change ", years[i], "-", years[j]), reverse = TRUE)) +
      theme_void() + theme(legend.background = element_rect(linetype = 1, size = 0.25, colour = 1), 
                           legend.margin = margin(.25, 0.25, 0.25, 0.25, unit='cm'),
                           legend.box.margin = margin(-7.8, 0, 0, -5.6, unit='cm'),
                           legend.title.align = 0.5)
    
    ggsave(plot = colour_map, width = 7, height = 6, dpi = 1200, filename = classified_out)
  }
}
